...
class IdempotencyStore:
    ...
    def claim(self, key: str, *, now: Optional[float] = None) -> tuple[bool, Optional[dict]]:
        ...
        Returns ``(True, None)`` when this caller won and should execute, or
        ``(False, record)`` when someone else already claimed it — where
        ``record`` carries the earlier outcome if it has settled, and a
        ``state`` of ``"in_flight"`` if it has not.

        An expired claim is reclaimable: a process that died mid-action must not
