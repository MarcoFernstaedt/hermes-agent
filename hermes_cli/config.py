...
def _normalize_max_turns_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "agent" in config or agent_config:
...
def save_config(
    ...
    *,
    ...
    with _config_transaction_lock():
        ...
        # The atomic write is authoritative. Refresh the source-isolated user
        # snapshot and invalidate merged/cache state before another reader runs.
        _SECURITY_USER_LKG_BY_PATH[path_key] = copy.deepcopy(
            persisted_user_expanded
        )
        _SECURITY_LKG_CONFIG_BY_PATH.pop(path_key, None)
...
def _read_user_config_for_mutation(config_path: Path) -> Dict[str, Any]:
    """Read raw user YAML for a locked mutation, failing closed on corruption."""
    require_readable_config_before_write(config_path)
    if not config_path.exists():
        return {}
    with open(config_path, encoding="utf-8") as f:
    ...
    if loaded is None:
        return {}
    if not isinstance(loaded, dict):
        raise ValueError("Refusing to mutate config.yaml: config root is not a mapping")
    return loaded
...
def _set_config_value_locked(
    ...
    *,
    ...
    user_config = _read_user_config_for_mutation(config_path)
    ...
    if coerce and isinstance(value, str) and not isinstance(_default_value_for_key(key), str):
...
def unset_config_value(key: str):
    """Remove a user-set configuration or .env value atomically."""
    with _config_transaction_lock():
        return _unset_config_value_locked(key)


def _unset_config_value_locked(key: str):
    """Implementation for ``unset_config_value``; caller holds the config lock."""
    if is_managed():
    ...
    # Use the authoritative save path so managed-policy checks, security LKGs,
    # caches, permissions, and durability are updated as one locked transaction.
    # Persist config.yaml before touching synchronized .env state: a failed
    # config write must leave the previously effective terminal setting intact.
