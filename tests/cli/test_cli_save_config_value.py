...
class TestSaveConfigValueAtomic:
    """save_config_value() must use the governed typed config writer."""
    ...
    def test_calls_authoritative_typed_setter(self, config_env, monkeypatch):
        """Legacy CLI writes must route through config policy and cache handling."""
        mock_update = MagicMock()
        monkeypatch.setattr("hermes_cli.config.set_config_value_typed", mock_update)

        from cli import save_config_value
        save_config_value("display.skin", "mono")

        mock_update.assert_called_once_with(
            "display.skin", "mono", config_path=config_env
        )
    ...
    def test_preserves_existing_values_after_config_mutation(self, config_env):
        """Authoritative writes may normalize YAML but must preserve values."""
    ...
    def test_file_not_truncated_on_error(self, config_env, monkeypatch):
        """If the authoritative setter raises, the original file is untouched."""
        original_content = config_env.read_text()

        def exploding_write(*args, **kwargs):
            raise OSError("disk full")

        monkeypatch.setattr("hermes_cli.config.set_config_value_typed", exploding_write)
    ...
    def test_managed_key_is_rejected(self, config_env, monkeypatch):
        """The legacy writer must not bypass managed-scope policy."""
        monkeypatch.setattr("hermes_cli.managed_scope.is_key_managed", lambda _key: True)
        monkeypatch.setattr("hermes_cli.managed_scope.get_managed_dir", lambda: None)

        from cli import save_config_value
        original_content = config_env.read_text()

        assert save_config_value("display.skin", "blocked") is False
        assert config_env.read_text() == original_content
